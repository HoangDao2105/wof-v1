...
export class PrimaryButton extends FancyButton {
    ...
    constructor(options: Partial<PrimaryButtonOptions> = {}) {
        ...
        let txt = opts.url? opts.url: Texture.from(opts.texture);
        if (!txt) {
        ...
        defaultView = new NineSliceSprite({
            ...
            width: opts.width,
            height: opts.height,
            tint:opts.textureColor?? primaryButtonOptions.textureColor
        });
        ...
        if (opts.icon && this.messageLabel) {
            this.icon = opts.iconUrl?Sprite.from(opts.iconUrl): Sprite.from(opts.icon);
            if (opts.text) {
